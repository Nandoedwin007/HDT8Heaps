...
public class VectorHeap<E extends Comparable<E>> implements PriorityQueue<E>
{
    ...
    protected void percolateUp(int leaf)
    {
        ...
        //Aqui cambiamos la comparacion de modo a que comparar solamente la letra final de cada linea de texto ingresada
        while (leaf > 0 &&
          (value.toString().substring(value.toString().length()-1).compareTo(data.get(parent).toString().substring(data.get(parent).toString().length()-1)) < 0))
        {
        ...
        while (root < heapSize) {
            ...
            if (childpos < heapSize)
            {
                if ((right(root) < heapSize) &&
                  ((data.get(childpos+1).toString().substring(data.get(childpos+1).toString().length()-1).compareTo
                   (data.get(childpos).toString().substring(data.get(childpos).toString().length()-1)) < 0)))
                {
                    childpos++;
                }
                // Assert: childpos indexes smaller of two children
                if ((data.get(childpos).toString().substring(data.get(childpos).toString().length()-1)).compareTo
                    (value.toString().substring(value.toString().length()-1)) < 0)
                {
